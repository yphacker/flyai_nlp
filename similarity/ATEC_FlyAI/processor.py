...
class Processor(Base):
    ...
    def input_x(self, texta, textb):
        ...
        if self.token is None:
            bert_vocab_file = os.path.join(DATA_PATH, "model", "chinese_L-12_H-768_A-12", 'vocab.txt')
            self.token = tokenization.CharTokenizer(vocab_file=bert_vocab_file)
        word_ids, word_mask, word_segment_ids = convert_single_example_simple(max_seq_length=64, tokenizer=self.token,
