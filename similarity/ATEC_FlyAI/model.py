...
class Model(Base):
    ...
    def predict(self, **data):
        ...
        with tf.Session() as session:
            ...
            pred = session.graph.get_tensor_by_name(self.get_tensor_name('pred'))
            x_input_ids, x_input_mask, x_segment_ids = self.data.predict_data(**data)
            predict = session.run(pred, feed_dict={input_ids: x_input_ids, input_mask: x_input_mask,
                                                   segment_ids: x_segment_ids, learning_rate: 0.0006})
