...
from os.path import MODEL_PATH
...
class Model(Base):
    ...
    def predict(self, **data):
        ...
        y_conv = self.session.graph.get_tensor_by_name(self.get_tensor_name('y_conv'))
        keep_prob = self.session.graph.get_tensor_by_name(self.get_tensor_name('keep_prob'))
        x_data = self.data.predict_data(**data)
        predict = self.session.run(y_conv, feed_dict={input_x: x_data, keep_prob: 1.0})
