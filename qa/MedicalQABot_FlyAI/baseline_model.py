...
class LSTM(object):
    def __init__(self, batch_size):
        ...
        # global_step = tf.Variable(0, name="global_step", trainable=False)
        # inputs, targets, lr, target_sequence_length, max_target_sequence_length, source_sequence_length = self.get_inputs()
        #
        # training_decoder_output, predicting_decoder_output = self.seq2seq_model(self.inputs,
        #                                                                    self.targets,
        #                                                                    self.target_sequence_length,
        ...
        #             tf.float32) * masks), tf.int32)
        # self.ans_accuracy = corr_target_id_cnt / tf.reduce_sum(target_sequence_length)
        #
        # with tf.name_scope("optimization"):
        #     self.cost = tf.contrib.seq2seq.sequence_loss(self.training_logits, targets, masks)
